...
--   Copyright (C) 2005  John T. O'Donnell
--   This is free software under the LGPL license; see the COPYING file.
--   The full source can be obtained at www.dcs.gla.ac.uk/~jtod/Hydra/
---------------------------------------------------------------------------

module HDL.Hydra.Core.Signal where
...
-- A data structure that describes where a name occurs within a
-- cluster.
...
-- The boxParent specifies where a circuit lies within the entire
-- circuit hierarchy.  It is either a top box (which means the box is
-- not a child of any known box; there may be several top boxes), or
-- else it is a child within a parent box, with an integer index
-- giving its position within the boxChildren list of the parent.
...
-- A structural signal contains an alternative semantics that
-- describes its behavior (for example, a stream).  This is returned
-- by the behavior function.
...
-- The constant signals can be defined as circuits with zero inputs
-- and one output.
